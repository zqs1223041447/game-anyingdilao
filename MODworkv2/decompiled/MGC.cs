...
public class MGC : MonoBehaviour
{
	...
	public void OnUserDefinedEvent(TrackEntry trackEntry, Spine.Event e)
	{
		...
		if (e.Data.Name == SPevent[0])
		{
			...
			{
				SingletonMonoScope<Gun>.Instance.MGCattack();
			}
		}
	...
	public void statMS()
	{
		if (FSM.Instance.GetMachine<StateMachine>("MGC") != null)
		{
			StateMachine.Destroy("MGC");
		}
		STA = StateMachine.Create("MGC").Build<State>("idle").OnEnter(delegate
		{
			idleON();
		})
			.OnStay(delegate
	...
	public void ACT(int anim)
	{
		...
		case 2:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(1, AttackC, loop: false);
			PLY.AttackTrack.TimeScale = PLY.SkillAnimationTimeScale;
			break;
		case 3:
			PLY.IsSkill = true;
