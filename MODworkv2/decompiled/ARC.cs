...
public class ARC : MonoBehaviour
{
	...
	public void OnUserDefinedEvent(TrackEntry trackEntry, Spine.Event e)
	{
		...
		if (e.Data.Name == SPevent[0])
		{
			...
			{
				SingletonMonoScope<Gun>.Instance.ARCattack();
			}
		}
		_ = e.Data.Name == SPevent[1];
		if (e.Data.Name == SPevent[2])
		{
			stat.SetEmptyAnimation(1, 0.5f);
		...
		_ = e.Data.Name == SPevent[14];
		_ = e.Data.Name == SPevent[15];
		_ = e.Data.Name == SPevent[16];
		...
		_ = e.Data.Name == SPevent[20];
		if (e.Data.Name == SPevent[21] && PLY.ChuanSong)
		{
			StartCoroutine(SingletonMonoScope<PlayerManager>.Instance.TeleportRoutine(PLY.ChuanSongPOS));
		}
	}
	...
	public void statMS()
	{
		if (FSM.Instance.GetMachine<StateMachine>("ARC") != null)
		{
			StateMachine.Destroy("ARC");
		}
		STA = StateMachine.Create("ARC").Build<State>("idle").OnEnter(delegate
		{
			idleON();
		})
			.OnStay(delegate
	...
	public void ACT(int an)
	{
		...
		{
		case 0:
			PLY.UpBOWTrack = stat.SetAnimation(1, upBOW, loop: true);
			PLY.AttackTrack = stat.SetAnimation(2, attackBOW, loop: true);
			PLY.UpBOWTrack.TimeScale = PLY.AttackAnimationTimeScale;
			PLY.AttackTrack.TimeScale = PLY.AttackAnimationTimeScale;
			break;
		case 1:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(2, attack4, loop: false);
			PLY.AttackTrack.TimeScale = PLY.AttackAnimationTimeScale;
			break;
		case 2:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(2, attack6, loop: false);
			PLY.AttackTrack.TimeScale = PLY.AttackAnimationTimeScale;
			break;
		case 3:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(2, attack8, loop: false);
			PLY.AttackTrack.TimeScale = PLY.AttackAnimationTimeScale;
			break;
		case 4:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(2, Hand, loop: false);
			PLY.AttackTrack.TimeScale = PLY.AttackAnimationTimeScale;
			break;
		case 5:
			PLY.IsSkill = true;
			...
			PLY.SkillTrack = stat.SetAnimation(3, skillA, loop: false);
			PLY.SkillTrack.TimeScale = PLY.SkillAnimationTimeScale;
			break;
		case 6:
			PLY.IsSkill = true;
			stat.AddEmptyAnimation(1, 0f, 0f);
			PLY.AttackTrack = stat.SetAnimation(2, skillB, loop: false);
