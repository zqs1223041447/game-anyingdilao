...
public class SK_Universe_Ball : MonoBehaviour
{
	...
	[HideInInspector]
	public SK_Universe_Aixs ownerAxis;
	...
	public void Stop(Transform trans, bool self)
	{
		...
		if (hasEXP)
		{
			Dicform component = ((!self) ? LeanPool.Spawn(EXP, base.transform.position, Quaternion.identity, trans) : LeanPool.Spawn(EXP, trans.position, Quaternion.identity)).GetComponent<Dicform>();
			component.sp = dic.sp;
