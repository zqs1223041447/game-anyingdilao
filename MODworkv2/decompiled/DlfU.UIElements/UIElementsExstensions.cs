...
public static class UIElementsExstensions
{
	...
	{
		ui.StyleDisplay((!value) ? DisplayStyle.None : DisplayStyle.Flex);
	}
	...
	{
		ui.StyleVisibility((!value) ? Visibility.Hidden : Visibility.Visible);
	}
	...
	{
		return ui.Children().ToList().Find((VisualElement childElement) => childElement.name == name);
	}
	...
	{
		return ui.hierarchy.Children().ToList().Find((VisualElement childElement) => childElement.name == name);
	}
