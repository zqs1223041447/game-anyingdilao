...
public class WeaponSaveData
{
	...
	public List<WPSkillSaveData> WPSK = new List<WPSkillSaveData>();

	public int MaxAocaoCount;

	public int AocaoCount;

	public List<WPAocaoSaveData> Aocao = new List<WPAocaoSaveData>();
	...
	public static WeaponSaveData FromRuntime(WeaponClass w)
	{
		...
		return new WeaponSaveData
		{
			...
			Craft_LockPrefix = w.Craft_LockPrefix,
			Craft_LockSuffix = w.Craft_LockSuffix,
			Craft_NoAttack = w.Craft_NoAttack,
			Craft_NoCaster = w.Craft_NoCaster,
			HHTime = w.HHCount,
			...
			WPSK = ((w.WPSK != null) ? w.WPSK.Select(WPSkillSaveData.FromRuntime).ToList() : new List<WPSkillSaveData>()),
			MaxAocaoCount = w.MaxAocaoCount,
			AocaoCount = w.AocaoCount,
			Aocao = ((w.Aocao != null) ? w.Aocao.Select(WPAocaoSaveData.FromRuntime).ToList() : new List<WPAocaoSaveData>()),
	...
	public void ApplyToRuntime(WeaponClass w)
	{
		...
		w.Craft_LockPrefix = Craft_LockPrefix;
		w.Craft_LockSuffix = Craft_LockSuffix;
		w.Craft_NoAttack = Craft_NoAttack;
		w.Craft_NoCaster = Craft_NoCaster;
		w.HHCount = HHTime;
		...
		w.WPSK = new List<WPSkill>();
		if (WPSK != null)
		{
			foreach (WPSkillSaveData item in WPSK)
			{
				if (item != null)
				{
					WPSkill wPSkill = new WPSkill();
					item.ApplyToRuntime(wPSkill);
					w.WPSK.Add(wPSkill);
				}
			}
		}
		...
		w.Aocao = new List<WPAocao>();
		if (Aocao != null)
		{
			foreach (WPAocaoSaveData item2 in Aocao)
			{
				if (item2 != null)
				{
					WPAocao wPAocao = new WPAocao();
					item2.ApplyToRuntime(wPAocao);
					w.Aocao.Add(wPAocao);
				}
			}
		}
		for (int i = 0; i < w.Aocao.Count; i++)
		{
			_ = w.Aocao[i];
		}
