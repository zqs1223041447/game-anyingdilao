...
public class SK_FSQ_fatherB : MonoBehaviour
{
	...
	public void FaShe()
	{
		...
		if (sp.Layer_SubA == 0 && sp.DamageA > 0f && SubA != null)
		{
			Dicform component14 = (PosSubA switch
			{
				0 => LeanPool.Spawn(SubA, PL.yao.transform.position, Quaternion.identity), 
				1 => LeanPool.Spawn(SubA, PL.transform.position, Quaternion.identity), 
				_ => LeanPool.Spawn(SubA, PL.yao.transform.position, Quaternion.identity), 
			}).GetComponent<Dicform>();
			component14.sp = sp;
			component14.SetCount(sp.ZY);
			component14.SubType = 1;
			component14.Index = 1;
		}
		if (sp.Layer_SubB == 0 && sp.DamageB > 0f && SubB != null)
		{
			Dicform component15 = (PosSubB switch
			{
				0 => LeanPool.Spawn(SubB, PL.yao.transform.position, Quaternion.identity), 
				1 => LeanPool.Spawn(SubB, PL.transform.position, Quaternion.identity), 
				_ => LeanPool.Spawn(SubB, PL.yao.transform.position, Quaternion.identity), 
			}).GetComponent<Dicform>();
			component15.sp = sp;
			component15.SetCount(sp.ZY);
			component15.SubType = 2;
			component15.Index = 1;
		}
	}
}
